...
import java.text.*;
class JavaQuiz implements QuizLayout
{
int i=1;
static int score=0;
JFrame f;
...
public void displayQues(String userid,int level,int qno,JLabel l2,JLabel l3,JRadioButton l4,JRadioButton l5,JRadioButton l6,JRadioButton l7)
{
	...
	PreparedStatement pst=con.prepareStatement("select * from quesjava where level=? and q_no=?");
	pst.setInt(1,level);
	pst.setInt(2,qno);
...
public void displayScore(String userid,int level)
{
	...
	if(level==1) 
		pst=con.prepareStatement("update quiz set j1= ? where userid=?");
	else if(level==2)
		 pst=con.prepareStatement("update quiz set j2= ? where userid=?");
	else
		 pst=con.prepareStatement("update quiz set j3= ? where userid=?");
